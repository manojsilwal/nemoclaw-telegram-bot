...
async function sendTelegramResult(chatId, issue, reportUrl) {
  ...
  return tgSend("sendMessage", {
    ...
    reply_markup: { inline_keyboard: [[
      ...
      { text: "🏢 Open Paperclip", url: PAPERCLIP_PUBLIC_URL },
    ]] },
  });
}
  ...
  try {
    ...
    for (const issue of issues) {
      ...
      try {
        ...
        const tg = await sendTelegramResult(chatId, full, reportUrl);
        if (tg.ok) console.log(`[poll] ✅ Notified chat ${chatId}`);
        else console.error(`[poll] ⚠️ TG error:`, tg.description);
        STATE.notified[issue.id] = { at: Date.now(), chatId };
        saveState(STATE);
